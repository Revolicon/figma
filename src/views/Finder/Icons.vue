<template>
  <ais-instant-search class="finder" :search-client="searchClient" index-name="icons">
    <ais-configure :facetFilters="[['style:line', 'style:brands']]" />
    <ais-search-box>
      <template v-slot="{ currentRefinement, isSearchStalled, refine }">
        <SearchBox :value="currentRefinement" :loading="isSearchStalled" :refine="refine" />
      </template>
    </ais-search-box>
    ...
    <ais-hits>
      <template v-slot="{ items, sendEvent }">
        <div class="empty" v-show="items.length < 1">
          ...
          <div class="empty__title">No icons found</div>
